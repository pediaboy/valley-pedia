...
export default function JokiRankPage() {
  const router = useRouter();
  return (
    <div className="min-h-screen px-4 py-8 max-w-2xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
        <h1 className="section-title mb-2" style={{ color: '#10b981' }}>Joki Rank</h1>
        <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.85rem' }}>Naik rank cepat & aman dengan tim profesional kami</p>
      </motion.div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {packages.map((pkg, i) => (
          <motion.div key={pkg.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
            ...
            onClick={() => router.push('/checkout?product=joki-rank-' + pkg.name.toLowerCase() + '&type=joki-rank')}>
            <div style={{ fontFamily: 'Orbitron', fontSize: '0.9rem', fontWeight: 700, color: '#10b981', marginBottom: '0.5rem' }}>{pkg.name}</div>
